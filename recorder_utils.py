...
Description: Added function headers to explain functions
            ...
            Added: make distance for distance_mode a custom input from user (as per task 2 requirements)
            Added value check for user input to limit within range 10 - 20
...
'''
Function converts adc values into voltage values that can be read by STM and to be used for amplitude plotting 
...
'''
Function plots data obtained into a graph of Amplitude against Time and saves it as a png file
...
'''
Function enables manual mode (custom user-set duration) to record audio in seconds. Processes incoming audio and saves data into raw & wav files.
...
trigger_dis = user set distance for the maximum distance to trigger recording
...
def distance_mode(ser):
    while True:
        try:
            trigger_dis = int(input("[INFO] Set Maximum Distance to trigger recording (Between 10 & 20 cm):\n ").strip())
            if 10 <= trigger_dis <= 20:
                break
            else:
                print("[ERROR] Please enter a value between 10 and 20.")
        except ValueError:
            print("[ERROR] Invalid input. Please enter an integer.")

    send_command(ser, str(trigger_dis)) 
    print(f"[INFO] Distance trigger mode activated. Will stop if the distance is further than {trigger_dis}cm for 5 seconds.\n")
